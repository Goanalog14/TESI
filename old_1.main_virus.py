...
def exe_inside_sandbox(virus,packer):
    ...
    try:
        ...
        if packer:
            data = [datetime.now(), get_bit(virus), get_sha1(virus).strip(), virus,result.stdout.strip()]
        else:
        ...
        if packer:
            data = [datetime.now(), get_bit(virus), get_sha1(virus).strip(), virus,e.stderr.strip()]
        else: 
            data = [datetime.now(), get_sha1(virus).strip(), virus,e.stderr.strip()]
    except TimeoutError:
        # Se il timeout viene raggiunto, restituisci "TIME EXCEEDED"
        if packer:
            data = [datetime.now(),get_bit(virus), get_sha1(virus).strip(), virus, "TIME EXCEEDED"]
        else:
